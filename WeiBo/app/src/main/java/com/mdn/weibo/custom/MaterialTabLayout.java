...
public class MaterialTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link MaterialTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link MaterialTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
...
//        mScrollableTabMinWidth = res.getDimensionPixelSize(R.dimen.design_tab_scrollable_min_width);
        // 设置tab的宽度 pengleiShen
        mScrollableTabMinWidth = res.getDimensionPixelSize(R.dimen.design_navigation_icon_padding);
    ...
    /**
     * Returns the current mode used by this {@link MaterialTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link MaterialTabLayout} with a {@link ViewPager}.
     * <p>
     * <p>This method will:
     * <ul>
     ...
     */
    public MaterialTabLayout setupWithViewPager(@NonNull ViewPager viewPager) {
     ...
     */
    public MaterialTabLayout setTabsFromPagerAdapter(@NonNull PagerAdapter adapter) {
        removeAllTabs();
    ...
    public static final class Tab {
        ...
        private final MaterialTabLayout mParent;

        Tab(MaterialTabLayout parent) {
            mParent = parent;
        }
     ...
     * necessary calls back to the provided {@link MaterialTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<MaterialTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(MaterialTabLayout tabLayout) {
        ...
        public void onPageScrolled(int position, float positionOffset,
            ...
            final MaterialTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
    ...
    private int mBottomLineColor = Color.parseColor("#dcdcdc");

    public MaterialTabLayout addTabs(String... tabs) {
        if (tabs == null || tabs.length == 0)
            return this;

        for (int i = 0; i < tabs.length; i++) {
            this.addTab(this.newTab().setText(tabs[i]).setTag(i));
        }
        return this;
    }

    public void setSelectTab(int tabPos) {
        getTabAt(tabPos).select();
    }

    public MaterialTabLayout setTabMargin(int margin) {
        mTabMargin = dpToPx(margin);
        return this;
    }
    ...
    public MaterialTabLayout setBottomLineColor(int lineColor) {
        this.mBottomLineColor = lineColor;
        return this;
    }


    public interface MaterialTabSelectedListener {
        void onTabSelected(Tab tab, boolean reSelected);
    }

    public MaterialTabLayout setOnMaterialTabSelectedListener(MaterialTabSelectedListener listener1) {
        this.listener = listener1;
        setOnTabSelectedListener(new MaterialTabSelectedAdapter());
      //  setSelectTab(0);
        return this;
    }

    private MaterialTabSelectedListener listener;

    public class MaterialTabSelectedAdapter implements OnTabSelectedListener {

        @Override
        public void onTabSelected(Tab tab) {
            if (listener != null) {
                listener.onTabSelected(tab, false);
                mLastCheckedPos = tab.getPosition();
            }
        }
        ...
        public void onTabReselected(Tab tab) {
            if (listener != null) {
                if (mLastCheckedPos == -1) {
                    ...
                    listener.onTabSelected(tab, true);
                }
            }
        }
    }
